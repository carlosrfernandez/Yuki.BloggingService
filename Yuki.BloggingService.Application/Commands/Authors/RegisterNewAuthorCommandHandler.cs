...
public class RegisterNewAuthorCommandHandler(IAggregateRepository repository) : ICommandHandler<RegisterNewAuthorCommand>
{
    private readonly IAggregateRepository _repository = repository ?? throw new ArgumentNullException(nameof(repository));
    
    public async Task HandleAsync(RegisterNewAuthorCommand command, CancellationToken cancellationToken = default)
    {
